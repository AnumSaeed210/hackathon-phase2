  ...
  return (
    <>
      ...
      <section className="relative bg-background py-20 sm:py-32 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-96 h-96 bg-accent-purple/30 rounded-full -mr-48 -mt-48 blur-3xl"></div>

        <div className="w-full max-w-6xl mx-auto relative z-10">
          {/* Section Header */}
          <div className="text-center max-w-3xl mx-auto mb-16 sm:mb-20">
            <h2
              className="text-4xl sm:text-5xl font-medium mb-6 tracking-tight text-cards"
              style={{
                color: "#4A4458",
            ...
            <p className="text-lg font-light leading-relaxed text-[#6B5D73]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {valueProps.map((prop, index) => (
              <div key={index} className="group relative text-center">
                {/* Number badge */}
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-accent-peach to-accent-yellow text-cards font-bold text-2xl mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg">
                  {prop.number}
                </div>
                ...
                <p className="text-[#6B5D73] leading-relaxed">
                  {prop.description}
                </p>

                {/* Optional: bottom accent line */}
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 h-1 w-0 bg-gradient-to-r from-transparent via-accent-purple to-transparent group-hover:w-full transition-all duration-500 rounded-full"></div>
              </div>
            ))}
          </div>
        </div>
